          ...
          {links.map(([href, label]) => {
            ...
            return href === "/systems" ? (
              <div className="nav-dropdown" key={href}>
                <Link href={href} className={`nav-link ${active ? "is-active" : ""}`}>{label}<ChevronDown size={15} /></Link>
                <div className="nav-dropdown-panel">
                  {systems.map(([slug, name]) => <Link key={slug} href={`/systems/${slug}`}>{name}</Link>)}
                </div>
              </div>
            ) : <Link key={href} href={href} className={`nav-link ${active ? "is-active" : ""}`}>{label}</Link>;
          })}
        </nav>
        <div className="header-actions">
          <a href={CONTACTS.phoneHref} className="header-phone"><Phone size={17} />{CONTACTS.phoneDisplay}</a>
          <Link href="/contacts#lead" className="button-primary">Получить консультацию</Link>
        </div>
      ...
      {open && (
        <div className="mobile-menu">
          <nav className="site-shell" aria-label="Мобильная навигация">
            ...
            <Link href="/contacts#lead" className="button-primary" onClick={() => setOpen(false)}>Получить консультацию</Link>
          </nav>
        </div>
      )}
    </header>
