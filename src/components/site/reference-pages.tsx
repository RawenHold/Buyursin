...
function ProjectAccordion({ projects, locale = "ru" }: { projects: readonly PortfolioProject[]; locale?: Locale }) {
  ...
  const labels = locale === "ru"
    ? { region: "Карусель проектов", type: activeProject.type, profile: "Профиль решения", discuss: "Обсудить похожий объект", all: "Проекты", next: "Следующий проект", playing: "Автопрокрутка", pause: "Остановить автопрокрутку", play: "Запустить автопрокрутку", visual: "Архитектурная AI-визуализация. Состав кейса подтверждается перед публикацией." }
    : { region: "Loyihalar karuseli", type: activeProject.typeUz, profile: "Yechim profili", discuss: "O‘xshash obyektni muhokama qilish", all: "Loyihalar", next: "Keyingi loyiha", playing: "Avto aylantirish", pause: "Avto aylantirishni to‘xtatish", play: "Avto aylantirishni ishga tushirish", visual: "Arxitektura AI-vizualizatsiyasi. Keys tarkibi nashrdan oldin tasdiqlanadi." };
  ...
  >
    ...
    <div className="cinematic-project-shell">
      <div className="cinematic-project-content" key={`${activeProject.title}-${locale}`}>
        ...
        <Link href="/contacts#lead" className="button-primary cinematic-project-cta" data-label={labels.discuss}>
          <span className="button-label">{labels.discuss}<ArrowRight /></span>
        </Link>
